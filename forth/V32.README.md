...
A sometimes  minimal freestanding `FORTH`  compiler and tutorial  for the
...
```
    assemble -o jonesforth.o jonesforth.s
...
implement a  few primitives, all  the rest  of the language  and compiler
is  written  in `FORTH`  itself.  Remember  I  said before  that  `FORTH`
lacked  **IF**-statements and  loops? Well  of course  it doesn't  really
because such  a lanuage would  be useless, but  my point was  rather that
**IF**-statements and loops are written in `FORTH` itself.
...
entry, commented in the LtU thread about this `FORTH`:
...
will  need  a  working  `Vircon32`  **DesktopEmulator**  along  with  its
**DevTools** installed.  This tutorial  will assume a  Linux command-line
environment. At least version **v25.10.19**, or later.

Again,  to  assemble  this  you  will  need  Vircon32's  **compile**  and
**assemble** tools from its **DevTools** suite.
...
about `assemble` (the **Vircon32 DevTools** assembler):
...
| R15      | 1111   | SP    | stack: stack pointer (top)   |
  ...
  * Register  names  run from  R0-R15.  All  are general  purpose  32-bit
    registers. Some have special purpose:
    * `R0`-`R10` (general purpose)
    * `R11` (string operation count register)
    * `R12` (string operation source register)
    * `R13` (string operation destination register)
    * `R14` (stack operation base pointer)
    * `R15` (stack operation stack pointer (top))
  * `IADD`, `MOV`, etc. take arguments in the form `DEST, SRC`
    * So `MOV R1, R0` moves `R0` -> `R1`
  * Constants/immediate values  are just  stated, decimal  or hexadecimal
    (prefixed with `0x` notation). Memory addresses are dereferenced with
    `[` and `]`:
    * `MOV R0, 2` moves the immediate value `2` into `R0`
    * `MOV R0, [2]` reads the 32-bit word from *memory address* `2` into
      `R0` (ie. possibly a mistake)

For more help reading the  assembler, reference the Vircon32 **DevTools**
documentation.
...
```
    f:
      CALL a            03 08 00 00 00
      CALL b            03 1C 00 00 00
      CALL c            03 2C 00 00 00
      ; ignore the return from the function for now
```

"**03**" is the `Vircon32` machine code  to "**CALL**" a function. In the
first 20 years  of computing memory was hideously expensive  and we might
have worried about  the wasted space being used by  the repeated "**03**"
bytes. We can save 20% in  code size (and therefore, in expensive memory)
...
On a `Vircon32`  machine it turns out that we  can write this interpreter
rather  easily, in  just a  few  assembly instructions.  Let's store  the
pointer to the next word to execute in the `R12` (**SR**) register:

```
       08 00 00 00 <- We're executing this one now
R12 -> 1C 00 00 00 <- this will be the _next_ one to execute
       2C 00 00 00
```

First, we read the memory at  `R12` into a general purpose register `R0`.
Second, we  increment the value (the  address) in `R12` by  `4` bytes. So
afterward, the situation now looks like this:

```
       08 00 00 00 <- We're still executing this one
       1C 00 00 00 <- R0 now contains this address (0x0000001C)
R12 -> 2C 00 00 00
```

Now we just  need to jump to the  address in `R0`. This is  just a single
`Vircon32` instruction  written `JMP R0`.  And after doing the  jump, the
...
### Vircon32 assembly: `NEXT` logic

To make this work, each subroutine is followed by the instructions:
...
Since  the `Vircon32`  **DevTools**  does not  have  support for  `macro`
directives, we will need to insert these instructions manually each time.
...
list". We also use  one register (`R12`) to act as  a kind of instruction
...
indirect `JMP`  to `R0` via  `LEA`. Take  the case where  we're executing
**DOUBLE** as  shown, and  **DUP** has  been called.  Note that  `R12` is
pointing to the address of **+**.

The assembly code for **DUP** eventually does a **NEXT**.  That:

  * reads the address of **+** into `R0` via `LEA`
    * `R0` points to the codeword of **+**
  * increments `R12` by `4`
  * jumps to the address in `R0`
...
My list of four things are:
  ...
  * which is related to the above is "how do you call into a function?"
    * ie. how does `R12` start  off pointing at part of `QUADRUPLE`,  but
...
that we save  the old `R12` (our "instruction pointer")  and create a new
one  pointing to  the first  word in  `DOUBLE`. Because  we will  need to
restore the old `R12` at the end  of `DOUBLE` (this is, after all, like a
...
about in the  previous paragraph, used to save `R12`  when calling from a
`FORTH` word into another `FORTH` word.

In this `FORTH`, we are using  the normal *stack pointer* (`R15`) for the
parameter stack.  We will use  the i386's "other" stack  pointer (`R14`),
called the "*base pointer*"), for our return stack.

I've got two  macros which just wrap  up the details of  using `R14` for
the return stack. You use them as for example "**PUSHRSP R0**" (`push R0`
on the  return stack) or  "**POPRSP R1**" (pop  top of return  stack into
`R1`).

### Vircon32 assembly routines for `PUSHRSP` and `POPRSP`

```
    ; PUSHRSP
    LEA   R14,     [R14-4]  ; push reg on to return stack
    MOV   R14,     reg

    ; POPRSP
    MOV   reg,     [R14]    ; pop top of return stack to reg
    LEA   R14,     [R14+4]
```
...
old `R12` on the stack and set `R12` to the first word in the definition.
Remember  that we  jumped to  the function  using `JMP  *(%eax)`? Well  a
consequence of that  is that conveniently `%eax` contains  the address of
this codeword,  so just by  adding `4`  to it we  get the address  of the
first data  word. Finally  after setting  up `R12`,  it just  does `NEXT`
which causes that first word to run.
...
DOCOL:
    ...
    IADD  R0,      4        ; R0 points to codeword, so make
    MOV   R12,     R0       ; R12 point to first data word
...
`DOCOL` does  this: It  pushes the  old `R12` on  the return  stack. `R0`
...
```
                ...
                +------------------+            DOUBLE:
                | addr of DOUBLE  ------------> +------------------+
top of return   +------------------+      R0 -> | addr of DOCOL    |
...
increments `R12` again and calls `DUP`.
...
_start:
    MOV   var_S0, R15         ; Save initial data stack pointer in FORTH variable S0.
    MOV   R14,    $return_stack_top  ; Initialise the return stack.
    CALL  _set_up_data_segment

    MOV   R12,    _cold_start  ; Initialise interpreter.
    ...
    .section .rodata
_cold_start:            ;  High-level code without a codeword.
    .int QUIT
```
